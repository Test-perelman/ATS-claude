...
# Execute RLS fix SQL directly against Supabase using psql over SSH
# This uses Supabase's direct database connection

DB_HOST="db.awujhuncfghjshggkqyo.supabase.co"
DB_PORT="5432"
DB_NAME="postgres"
DB_USER="postgres"
DB_PASS="Z4YlRW8HHXfMPdqe"

# Create SQL file
cat > /tmp/rls_fix.sql << 'SQL'
...
RETURNS BOOLEAN AS $$
  SELECT COALESCE((SELECT r.is_admin FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = user_id), FALSE)
$$ LANGUAGE sql STABLE SECURITY DEFINER;
